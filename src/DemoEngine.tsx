...
// Helper to get relevant images from LoremFlickr based on category keyword
const getImage = (keyword: string, w: number, h: number, lock: number) => 
  `https://loremflickr.com/${w}/${h}/${keyword.split(" ")[0].toLowerCase()}?lock=${lock}`;
  ...
  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="pt-32 px-8 max-w-7xl mx-auto min-h-screen">
    <div className="flex flex-col md:flex-row items-center gap-16">
      <div className="flex-1 space-y-8">
        ...
        <Link to="about" className="inline-block px-8 py-4 bg-white text-black font-bold rounded-full hover:scale-105 transition-transform">Discover More</Link>
      </div>
      <div className="flex-1">
        <img src={getImage(cat, 800, 600, 1)} alt="Hero" className="rounded-3xl shadow-2xl shadow-blue-500/20 w-full object-cover aspect-video" />
      </div>
    </div>
  </motion.div>
);
...
const About = ({ cat }: { cat: string }) => (
  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="pt-32 px-8 max-w-4xl mx-auto min-h-screen text-center">
    ...
    <img src={getImage(cat, 1200, 500, 2)} alt="About" className="rounded-3xl mb-12 w-full h-64 object-cover grayscale hover:grayscale-0 transition-all duration-700" />
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
      {[1, 2, 3].map((i) => (
        <div key={i} className="bg-neutral-900 border border-white/5 rounded-3xl p-8 hover:bg-neutral-800 transition-colors">
          <img src={getImage(cat, 400, 300, i + 10)} className="rounded-xl w-full h-48 object-cover mb-6" alt={`Service ${i}`} />
  ...
  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="pt-32 px-8 max-w-7xl mx-auto min-h-screen">
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
        <img key={i} src={getImage(cat, 500, 500, i + 20)} className={`rounded-2xl object-cover w-full h-full ${i === 1 || i === 4 ? "col-span-2 row-span-2" : ""}`} alt="Gallery" />
      ))}
    </div>
  </motion.div>
);
  ...
  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="pt-32 px-8 max-w-4xl mx-auto min-h-screen">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-12 bg-neutral-900 p-12 rounded-3xl border border-white/5">
      ...
      <form className="flex flex-col gap-6">
        ...
        <button type="button" className="bg-blue-600 hover:bg-blue-500 text-white font-bold py-4 rounded-xl transition-colors">Send Message</button>
      </form>
    </div>
  </motion.div>
);
    ...
    <div className="bg-black min-h-screen text-white pb-20">
      ...
      <nav className="fixed top-0 w-full bg-black/80 backdrop-blur-md border-b border-white/5 z-50 px-8 py-4 flex items-center justify-between">
        ...
        <div className="hidden md:flex gap-8">
          {navLinks.map(link => {
            // Check if current path matches link path exactly, or ends with it
            const isActive = location.pathname === link.path || location.pathname === link.path + "/";
            return (
              <Link key={link.name} to={link.path} className={`text-sm uppercase tracking-widest transition-colors ${isActive ? "text-blue-400 font-bold" : "text-neutral-400 hover:text-white"}`}>
                {link.name}
              </Link>
            );
          })}
