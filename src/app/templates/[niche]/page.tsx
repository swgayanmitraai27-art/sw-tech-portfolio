        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 w-full pt-20">
          <div className="max-w-3xl space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-1000 fill-mode-both">
            ...
            <div className="flex flex-wrap items-center gap-4 pt-4">
              <a href={/templates//contact} className="px-8 py-4 rounded-xl bg-white text-slate-950 font-bold hover:bg-slate-200 transition-all duration-300 transform hover:scale-105 shadow-2xl">
                Initialize Build
              </a>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {data.features && data.features.map((feat: any, idx: number) => (
              <div key={idx} className="group relative bg-slate-900/50 backdrop-blur-sm border border-slate-800 rounded-3xl p-10 hover:bg-slate-900 transition-colors">
                <div className={bsolute top-0 right-0 w-64 h-64 bg- + data.accent + /5 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 group-hover:bg- + data.accent + /10 transition-colors} />
